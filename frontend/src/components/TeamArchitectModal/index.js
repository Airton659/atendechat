...
const TeamArchitectModal = ({ open, onClose, onSave }) => {
  ...
  const handleGenerate = async (values) => {
    setGenerating(true);
    try {
      const { data } = await api.post("/teams/generate", values);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // A equipe e agentes já foram salvos no PostgreSQL pelo backend durante generate
  ...
  const renderAgentsTab = () => {
    ...
    const agents = Array.isArray(generatedTeam.team.agents)
      ? generatedTeam.team.agents
      : Object.values(generatedTeam.team.agents);
  ...
  const renderToolsTab = () => {
    if (!generatedTeam?.team?.customTools || generatedTeam.team.customTools.length === 0) {
      return (
        <Typography variant="body2" color="textSecondary">
    ...
    return (
      <List>
        {generatedTeam.team.customTools.map((tool, idx) => (
          <React.Fragment key={idx}>
            <ListItem className={classes.agentItem}>
              <ListItemText
                primary={
              ...
              <DialogContent dividers>
                ...
                <div className={classes.root} style={{ marginTop: 16 }}>
                  <Field
                    ...
                    placeholder="Ex: Equipe de Vendas, Suporte ao Cliente..."
                  />
                </div>
...
export default TeamArchitectModal;
