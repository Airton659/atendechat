...
const AgentArchitectModal = ({ open, onClose, onSave }) => {
  const classes = useStyles();
  const [step, setStep] = useState(1);
  const [generating, setGenerating] = useState(false);
  const [saving, setSaving] = useState(false);
  const [generatedAgents, setGeneratedAgents] = useState(null);
  ...
  const handleGenerate = async (values) => {
    setGenerating(true);
    try {
      const { data } = await api.post("/agents/generate", values);
      setGeneratedAgents(data);
      setStep(2);
      toast.success("Agentes gerados com sucesso!");
    } catch (err) {
      toastError(err);
    } finally {
      setGenerating(false);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Os agentes já foram salvos no PostgreSQL pelo backend durante generate
  ...
  const renderAgentsTab = () => {
    if (!generatedAgents?.blueprint?.agents) {
      return (
    ...
    const agents = Array.isArray(generatedAgents.blueprint.agents)
      ? generatedAgents.blueprint.agents
      : Object.values(generatedAgents.blueprint.agents);
            ...
            <ListItem className={classes.agentItem}>
              <ListItemText
                ...
                secondary={
                  <>
                    <Typography variant="body2" color="textSecondary">
                      <strong>Função:</strong> {agent.function || agent.role}
                    </Typography>
                    <Typography variant="body2" color="textSecondary">
                      <strong>Objetivo:</strong> {agent.objective || agent.goal}
                    </Typography>
                    <Typography variant="body2" color="textSecondary">
                    ...
                    {agent.keywords && agent.keywords.length > 0 && (
                      <Typography variant="body2" color="textSecondary">
                        <strong>Keywords:</strong> {agent.keywords.join(", ")}
                      </Typography>
                    )}
                  </>
                }
  ...
  const renderToolsTab = () => {
    if (!generatedAgents?.blueprint?.customTools || generatedAgents.blueprint.customTools.length === 0) {
      return (
        <Typography variant="body2" color="textSecondary">
    ...
    return (
      <List>
        {generatedAgents.blueprint.customTools.map((tool, idx) => (
          <React.Fragment key={idx}>
            <ListItem className={classes.agentItem}>
              <ListItemText
                primary={
    ...
    >
      <DialogTitle>
        <Box display="flex" alignItems="center" gap={1}>
          <AutoAwesomeIcon />
          <span>Gerar Agentes com IA</span>
        </Box>
      </DialogTitle>
            ...
            <Form>
              <DialogContent dividers>
                <Typography variant="body2" color="textSecondary" gutterBottom>
                  Descreva seu negócio e deixe a IA criar agentes especializados para você
                </Typography>
      ...
      {step === 2 && generatedAgents && (
        <>
          <DialogContent dividers>
            <Typography variant="body2" color="textSecondary" gutterBottom>
              Agentes gerados com sucesso! Revise os agentes antes de finalizar.
            </Typography>
...
export default AgentArchitectModal;
